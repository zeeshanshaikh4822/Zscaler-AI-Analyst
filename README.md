...
>
> **Always use a read-only API credential** (see [Zscaler API Setup](#zscaler-api-setup)). This tool only performs GET requests and never modifies your configuration, but a scoped read-only key ensures that it's enforced at the API level.
